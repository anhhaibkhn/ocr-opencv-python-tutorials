""" This file accepts an input image from disk, loads the trained RFR, and then denoises the input image. """
# import the necessary packages
from config import denoise_config as config
from pyimagesearch.denoising.helpers import blur_and_threshold
...
ap.add_argument("-s", "--sample", type=int, default=10, help="sample size for testing images")
args = vars(ap.parse_args())

# load our document denoiser - RFR model from disk
print("[INFO] loading RFR model...")
model = pickle.loads(open(config.MODEL_PATH, "rb").read())

# grab the paths to all images in the testing directory and then randomly sample them
print("[INFO] sampling testing images...")
...
for imagePath in imagePaths:
    ...
    # initialize a list to store our ROI features (i.e., 5x5 pixel neighborhoods)
    roiFeatures = []
    ...
    for y in range(0, image.shape[0]):
        for x in range(0, image.shape[1]):
            ...
            if rW != 5 or rH != 5:
                continue
            # our features will be the flattened 5x5=25 pixels from the training ROI
    ...
    # the pixels list is currently a 1D array so we need to reshape it to a 2D array 
    # (based on the original input image dimensions) and then scale the pixels from the range [0, 1] to [0, 255]
